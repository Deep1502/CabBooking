...
namespace CabBooking
{
    public partial class adminlogin : System.Web.UI.Page
    {
        ...
        protected void btn_Login_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT COUNT(*) FROM Admin WHERE Email = '" + tb_Email.Text + "' AND Password = '" + tb_Password.Text + "'";
            try
            {
                using (con)
                {
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        ...
                        if (count == 1)
                        {
                            string idquery = "SELECT Id FROM Admin WHERE Email = '" + tb_Email.Text + "' AND Password = '" + tb_Password.Text + "'";
